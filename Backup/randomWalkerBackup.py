...
class KG2RelPattern():
    ...
    def extractor(self, graph: DGLGraph, verbose=1):
        ...
        ## step 4)
        if verbose:
            print("Step 4: Relation to node pairs...")
        group_time = time.time()
        #{n_0; n_{walk number}} {walk_len;walk_pair;start_node;end_node;rw_count;} {r_0;r_{walknumber-1}}
        relation_walks = self.relation_path_to_node_pairs(walks)
        if verbose:
            print("{} relations map to the entitiy pairs in {:.2f} seconds".format(walks.shape[0], time.time() - group_time))
        return walks, relation_walks
    ...
    def relation_path_to_node_pairs(self, walks: DataFrame):
        relation_names = ['r_' + str(idx) for idx in range(self.walklen - 1)]
        relation_groups = walks.groupby(relation_names)
        relation2node_pairs = []
        for relation, group in relation_groups:
            relation_count = group['rw_count'].sum()
            node_pairs = group[['start_node', 'end_node', 'rw_count']].to_numpy().tolist()
            relation_len = group['walk_len'].values[0]
            relation2node_pairs.append((relation, relation_len, relation_count, node_pairs))
        relation_walks = pd.DataFrame(data=relation2node_pairs, columns= ['p', 'p_len', 'p_cnt', 'p_pir'])
        return relation_walks