...
class TrainDataset(Dataset):
    ...
    def triple2path(self, pos_samp, neg_samp):
        ...
        path_num_array = np.zeros(num_of_triples, dtype=np.int64)
        ...
        head_in_data, head_in_num, head_out_data, head_out_num = head_data['ir_path'].to_numpy(), head_data[
            'i_num'].to_numpy(), \
                                                                 head_data['or_path'].to_numpy(), head_data[
                                                                     'o_num'].to_numpy(),
        tail_in_data, tail_in_num, tail_out_data, tail_out_num = tail_data['ir_path'].to_numpy(), tail_data[
            'i_num'].to_numpy(), \
                                                                 tail_data['or_path'].to_numpy(), tail_data[
                                                                     'o_num'].to_numpy()
        head_data_list, head_num_array, tail_data_list, tail_num_array = [[] for _ in range(2 * num_of_triples)], \
                                                                         np.zeros(2 * num_of_triples, dtype=np.int32), \
                                                                         [[] for _ in range(2 * num_of_triples)], \
                                                                         np.zeros(2 * num_of_triples, dtype=np.int32)

        for i in range(num_of_triples):
            head_data_list[2 * i], head_data_list[2 * i + 1] = np.array(head_in_data[i]), np.array(tail_in_data[i])
            head_num_array[2 * i], head_num_array[(2 * i + 1)] = head_in_num[i], tail_in_num[i]
            tail_data_list[2 * i], tail_data_list[2 * i + 1] = np.array(tail_out_data[i]), np.array(head_out_data[i])
            tail_num_array[2 * i], tail_num_array[(2 * i + 1)] = tail_out_num[i], head_out_num[i]
        head_data_array, tail_data_array = np.concatenate(head_data_list, axis=0), np.concatenate(tail_data_list,
                                                                                                  axis=0)
        ###############################################
        ...
        head_idxs, tail_idxs, rel_idxs = np.full(total_path_num, fill_value=-1, dtype=np.int64), np.full(total_path_num,
                                                                                                         fill_value=-1,
                                                                                                         dtype=np.int64), \
        ...
        rel_idxs[:pair_num_cum[1]] = 0
        for i in range(1, 2 * num_of_triples):
            head_idxs[pair_num_cum[i - 1]:pair_num_cum[i]] = np.repeat(np.array(range(head_num_array[i])),
                                                                       tail_num_array[i]) + head_bias[i - 1]
            tail_idxs[pair_num_cum[i - 1]:pair_num_cum[i]] = np.array(
                list(range(tail_num_array[i])) * head_num_array[i]) + \
                                                             tail_bias[i - 1]
            if i % 2 == 1:
                path_num_array[(i - 1) // 2] = pair_num[i] + pair_num[i - 1]
            if i % 2 == 0:
                rel_idxs[pair_num_cum[i - 1]:pair_num_cum[i + 1]] = i // 2
        #########Generate path#########################
        head_paths = head_data_array[head_idxs]
        rel_path = relations[rel_idxs]
        tail_paths = tail_data_array[tail_idxs]
        CLS_path, SEP_path = np.full((total_path_num, 1), fill_value=CLS, dtype=np.int64), np.full((total_path_num, 1),
                                                                                                   fill_value=SEP,
                                                                                                   dtype=np.int64)
        paths = np.concatenate([CLS_path, head_paths, rel_path.reshape(total_path_num, 1), tail_paths, SEP_path],
                               axis=1)
        #########Generate path#########################
        #########Generate path#########################
...
class TestDataset(Dataset):
    ...
    def triple2path(self, pos_samp, neg_samp):
        ...
        head_data, relations, tail_data = self.ent2path_df.loc[pn_data[:,0], :], \
                                          pn_data[:,1], self.ent2path_df.loc[pn_data[:,2], :]
        head_in_data, head_in_num, head_out_data, head_out_num = head_data['ir_path'].to_numpy(), head_data['i_num'].to_numpy(), \
                                                                 head_data['or_path'].to_numpy(), head_data['o_num'].to_numpy(),
        tail_in_data, tail_in_num, tail_out_data, tail_out_num = tail_data['ir_path'].to_numpy(), tail_data['i_num'].to_numpy(), \
        ...
        for i in range(num_of_triples):
            ...
            tail_data_list[2*i], tail_data_list[2*i + 1] = np.array(tail_out_data[i]), np.array(head_out_data[i])
            tail_num_array[2*i], tail_num_array[(2*i+1)] = tail_out_num[i], head_out_num[i]
        head_data_array, tail_data_array = np.concatenate(head_data_list, axis=0), np.concatenate(tail_data_list, axis=0)
        ###############################################
        #########Generate path level indexes###########
        head_bias, tail_bias = np.add.accumulate(head_num_array), np.add.accumulate(tail_num_array)
        pair_num = head_num_array * tail_num_array
...
#     # start = time()
    ...
    for i in range(1000):
        ...
        print(batch['path'].shape)
        # paths, path_mask, path_position, path_batch_num, path_total_num, weight, mode = batch['path'], batch[
        #     'path_mask'], \
